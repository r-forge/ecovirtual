...
nt=dim(resulta)[1]-1
old=par(mfrow=c(2,1), mar=c(2,4,2,2), cex.lab=0.7, cex.axis=0.7,cex.main=0.8)
...
plot(resulta[1:nt,"time"] , resulta[1:nt,"Nt.dt"],main="Population Size", type="l",xlab="time", ylab="Number of individuos",lty=2)
...
legend("topleft", colnames(resulta)[2:3],lty=2,col=c(1,2),bty="n")
abline(h=K, lty=3, col="blue")
text(x=tmax/2, y=K+2, "Carying capacity", col="blue")
text(x=tmax*0.4, y= resulta[(tmax/2),2], paste("r=", r))
invisible(resulta)
}
...
#teste1=popStr(p.sj=0.05, p.jj=0.99, p.ja=0, p.aa=1, fec=1.2, ns=100,nj=150,na=50, ln=20, cl=20, tmax=100)
