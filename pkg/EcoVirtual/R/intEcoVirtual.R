...
ln=dim(dados)[1]
cl=dim(dados)[2]
layout(matrix(c(2,1), ncol=1, nrow=2), heights=c(5,2),widths=c(1,1))
plot(1:10,1:10, type="n", mar=c(1,0,1,0), xaxt="n", yaxt="n", bty="n", xlab="", ylab="", cex=0.8)
legend(1, 5,ncol=4, legend=c("not available", "empty", "species 1", "species 2"), pch=c(15,22,15,15), title="Patches legend", col=c("red","black", "blue", "green"),bty="n")
#layout.show()
image(0:ln, 0:cl, dados[,,1], col=c("red", "white","blue" ,"green") , breaks=c(-0.1,-0.001,0.1,1.9,2.9),main="Metapopulations Competition", sub=paste("Initial configuration from", nsim," simulations",  sep=""), xlab="", ylab="")	
grid(ln,cl)
Sys.sleep(.5)
	for(i in 2:nsim)
	{
image(0:ln, 0:cl, dados[,,nsim], col=c("red", "white","blue" ,"green") , breaks=c(-0.1,-0.001,0.1,1.9,2.9),main="Metapopulations Competition", sub=paste("Simulation no.", i,"; total", nsim,   sep=""), xlab="", ylab="")	
	Sys.sleep(.1)
	}
}
###############################################
